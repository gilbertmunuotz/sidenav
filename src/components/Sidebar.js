...
function Sidebar({ children }) {
    ...
    return (
        <div className="container flex">
            <div className={applyClasses(isOpen, 'sidebar bg-black text-white h-screen w-64 transition-all duration-500 ease-in-out transform', 'w-16')}>
                ...
                {menuItems.map((item, index) => (
                    <NavLink key={index} to={item.path}
                        className={applyClasses(isOpen, 'link flex items-center py-3 px-6 gap-4 hover:bg-[#495057] rounded-lg hover:text-white', 'hover:bg-transparent')}>
